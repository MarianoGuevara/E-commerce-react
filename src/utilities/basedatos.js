...
import { appInit } from "./firebase.js";
...
export async function getSome(coleccion, condicion) {
	const ref = collection(bd, coleccion);
	const q = query(ref, condicion); 
	const queryEjecutada = await getDocs(q);
	return queryEjecutada.docs;
}
...
export async function obtenerKeyCasteada(coleccion, key) {
	const queryEjecutada = await getAll(coleccion);
	const array = queryEjecutada.map(doc => doc.data()[key]);
...
export async function obtenerWhereCasteada(coleccion, key, value) {
	...
	const queryEjecutada = await getSome(coleccion, condicion);
	return castearFirebaseObj(queryEjecutada);
}
...
//     const docRef = doc(bd, collectionName, docId);  // crear referencia al doc
//     await setDoc(docRef, product);                  // subir el producto
//     console.log(`Producto ${docId} subido.`);
//   }
